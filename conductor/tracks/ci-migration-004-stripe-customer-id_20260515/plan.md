...
- [ ] Task 1.1: Read `backend/migrations/004_performance_indexes.sql:12` plus surrounding context (full statement)
- [ ] Task 1.2: `grep -rn "stripe_customer_id" backend/migrations/` to find which migration actually creates the column. Note its sequence number.
- [ ] Task 1.3: Query prod schema for `stripe_customer_id` column's current state (which table, type, nullable)
- [ ] Task 1.4: Decide: reorder, guard, or merge migrations

## Phase 2: Patch

- [ ] Task 2.1: Apply the chosen fix. Most likely shape:
...
- [ ] Task 2.2: Add header comment block + link to this track

## Phase 3: Verify + ship

- [ ] Task 3.1: Docker postgres:16-alpine fresh replay of migrations 001..068 — all green
- [ ] Task 3.2: Commit + push, Migration Validation apply-from-scratch step green
- [ ] Task 3.3: Verify prod schema unchanged after next deploy

## Completion Criteria

- [ ] `psql -v ON_ERROR_STOP=1 -f` cleanly applies migrations 001..068 against empty DB
- [ ] Prod schema unchanged
- [ ] Migration Validation CI job fully green on main
