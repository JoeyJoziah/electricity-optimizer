...
| [~] | ci-frontend-lint-workflow_20260515 | CI Frontend Lint — ✅ MIGRATION MERGED 2026-05-15 (commit `21f85b87`). ESLint 8.56→9.17, flat-config via `eslint-config-next/core-web-vitals` (native flat export, no FlatCompat needed), custom rules re-expressed in flat-config blocks, ci.yml updated. `npm run lint` runs cleanly. **CI Frontend Lint will still show RED** because 95 latent errors + 16 warnings surfaced — pre-existing debt that the `next lint` crash had been masking for ≥15 days. Cleanup tracked in `ci-frontend-lint-baseline_20260515`. **Awaiting CI confirmation that lint pipeline executes (vs crashes).** | 2026-05-15 | 2026-05-15 |
...
| [~] | ci-migration-004-stripe-customer-id_20260515 | CI Migration Apply (continued) — ✅ FIX MERGED 2026-05-15 (commit `1fd2d1eb`). Pattern A (DO-block guard) applied to BOTH `004_performance_indexes.sql` AND `037_additional_performance_indexes.sql` since both reference the column. Discovered `users.stripe_customer_id` is actually created in migration 057 (out-of-order in prod history). Idempotent on prod + skips on fresh replay (NOTICE: index deferred to 057). **Awaiting CI green.** ⚠️ Agent discovered **6 more latent reds** in the migration chain — see `ci-migration-chain-replay_20260515` | 2026-05-15 | 2026-05-15 |
| [~] | ci-pandas-py312_20260515 | CI ML Tests (continued) — ✅ FIX MERGED 2026-05-15 (commit `f81d0d24`). Bumped `pandas==2.1.0` → `pandas==2.1.1` in `ml/requirements.txt`. Pulled cp312-cp312-macosx_11_0_arm64 wheel cleanly. Verified compatible with pinned `numpy==1.26.0`. 16 pandas touchpoints in `ml/` — patch bump = bugfix-only, no API surface change. **Awaiting CI green.** | 2026-05-15 | 2026-05-15 |
| [ ] | ci-frontend-lint-baseline_20260515 | Frontend Lint Baseline Cleanup — 95 errors + 16 warnings surfaced post-migration. Breakdown: 39 `no-unused-vars`, 35 `react/display-name`, 15 `no-require-imports` (auto-fixable), 3 `react-hooks/rules-of-hooks` (real bugs), 15 `no-console` warnings, 4 misc. 5-phase plan (auto-fix → real bugs → mechanical → console → ship). Last Frontend Lint CI green gate for `ph-relaunch-jun2_20260515` | 2026-05-15 | 2026-05-15 |
| [ ] | ci-migration-chain-replay_20260515 | CI Migration Chain Replay — 6 additional latent reds discovered 2026-05-15 by `ci-migration-004-stripe-customer-id` agent during full-chain Docker replay: (1) `init_neon.sql:160` column "timestamp" missing; (2) `017:30` column "status" missing; (3) `035` `neon_auth.user` relation missing (Neon-only schema); (4) `037:15` column "utility_type" missing on `electricity_prices`; (5) `049, 051, 053, 059, 061` schema-shape errors (constraint/trigger conflicts, non-IMMUTABLE function in index); (6) pervasive `role "neondb_owner" does not exist` (environment, not migration). Last full-chain CI-green gate for `ph-relaunch-jun2_20260515`. **Likely needs splitting into per-migration tracks once triaged.** | 2026-05-15 | 2026-05-15 |
