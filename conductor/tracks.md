...
| [x] | ci-red-triage_20260515 | CI Red Triage — all 4 phases done + all 5 split latent-red children fixed. **Phase 3 npm audit RESOLVED 2026-05-21 via allow-list** (`.github/scripts/check_npm_audit.py`; Security Scan job success on push `be6f6fd3`; documented `next`/`kysely` exceptions, review 2026-07-01; blanket `npm audit fix` reverted as non-targeted). | 2026-05-15 | 2026-05-21 |
...
| [x] | ci-ml-tensorflow-py312_20260515 | CI ML Tests — **GREEN 2026-05-21** (ML Tests job success on push `3060d11d`). Install fix (TF 2.17.1 cp312) + Keras-3 migration of CNN-LSTM (MedianMAE metric for the 3-quantile `[?,24,3]` output, register_keras_serializable, lower<=upper, fig.savefig). 3 slow training tests marked `@pytest.mark.slow` (CI CPU >120s timeout). `ml/tests/` = 723 passed / 0 failed; `testpaths=tests` excludes script-style `test_forecaster.py`. | 2026-05-15 | 2026-05-21 |
...
| [x] | ci-frontend-lint-baseline_20260515 | Frontend Lint Baseline Cleanup — **DONE + CI GREEN 2026-05-21** (Frontend Lint job success on push `588370c5`). `npm run lint` → 0 errors (from 91); 15 no-console warnings retained; full FE suite 3437/3437; config untouched, no rules disabled. 17 react-hooks errors were all REAL bugs fixed at root cause (none Playwright FPs); 39 no-unused-vars; 35 react/display-name fixed by naming wrappers. | 2026-05-15 | 2026-05-21 |
| [x] | ci-migration-chain-replay_20260515 | CI Migration Chain Replay — **FIXED + CI GREEN 2026-05-21** (Migration Validation job success on push `588370c5`, after also provisioning `neondb_owner` in ci.yml). Neon ephemeral-branch replay was 68/68. Triaged via prod-fork ground truth: real failures were 017 (×2: status, utility_type), 035 (neon_auth.user), 053 + 062 (DATE(timestamptz) not IMMUTABLE), 056 (stripe_customer_id), 061 (model_name→model_version, INFERRED). Original report's 049/051/059 were Docker CONCURRENTLY false-positives; `neondb_owner` was a Docker-env artifact (exists on real Neon). 6 files edited, all additive/guarded (prod-safe). 2 flags: 061 inferred-intent; idempotency re-run fails on 6 OTHER untouched migrations (pre-existing, not CI-tested). | 2026-05-15 | 2026-05-21 |
