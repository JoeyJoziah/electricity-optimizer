...
async def get_model_accuracy(redis_client) -> float | None:
    ...
    if model is not None:
        try:
            ...
            try:
                ...
                if redis:
                    await redis.set("model:last_updated", datetime.now(UTC).isoformat())
            except Exception:
                pass
...
async def predict_prices(
    ...
    **Example**:
    ...
    try:
        ...
        logger.exception("forecast_generation_failed", error=str(e))
        raise HTTPException(
    ...
    **Example**:
    ...
    try:
        ...
        if request.earliest_start or request.latest_end:
            earliest = request.earliest_start or datetime.now(UTC)
            latest = request.latest_end or (datetime.now(UTC) + timedelta(days=1))
        ...
        logger.exception("optimal_times_failed", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Prediction failed. See server logs for details.",
        )
...
async def estimate_savings(
    ...
    **Example**:
    ...
    try:
        ...
        logger.exception("savings_estimate_failed", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Prediction failed. See server logs for details.",
        )
...
async def get_model_info(
    ...
    return {
        ...
        "last_updated": last_updated or datetime.now(UTC).isoformat() + "Z",
    }
