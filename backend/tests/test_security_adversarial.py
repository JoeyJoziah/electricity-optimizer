...
def test_app():
    ...
    async def _test_bearer_auth(request: Request):
        ...
        if not token:
            raise HTTPException(status_code=401, detail="Not authenticated")
        try:
            payload = jose_jwt.decode(token, TEST_SECRET, algorithms=[TEST_ALGORITHM])
...
def full_app_client():
    ...
    with patch("config.database.db_manager.initialize", new_callable=AsyncMock), patch(
        "config.database.db_manager.close", new_callable=AsyncMock
    ), patch(
        ...
        "config.database.db_manager.get_timescale_session"
    ) as mock_session_cm:
...
class TestSQLInjection:
    ...
    def test_region_sql_injection_returns_422(self, full_app_client):
        ...
        for payload in SQL_INJECTION_PAYLOADS:
            response = full_app_client.get(f"/api/v1/prices/current?region={payload}")
    ...
    def test_days_parameter_sql_injection(self, full_app_client):
        ...
        for payload in ["1; DROP TABLE--", "' OR 1=1", "1 UNION SELECT 1"]:
            ...
            assert response.status_code == 422, (
                f"Expected 422 for non-integer 'days={payload}', " f"got {response.status_code}."
            )
    ...
    def test_xss_in_name_field_accepted_or_rejected_cleanly(self, client):
        ...
        for payload in XSS_PAYLOADS:
            ...
            assert response.status_code in (200, 422, 400), (
                f"XSS payload in 'name' field caused unexpected status " f"{response.status_code}."
            )
...
class TestCORSValidation:
    ...
    def test_allowed_origin_receives_cors_header(self, client):
        ...
        assert "localhost:3000" in acao or acao == "*", (
            f"Allowed origin did not receive ACAO header. " f"Got: {acao!r}"
        )
...
class TestStripeWebhookSecurity:
    ...
    def test_webhook_no_signature_header_returns_400(self, full_app_client):
        ...
        assert response.status_code == 400, (
            f"Webhook without signature header returned {response.status_code}, " f"expected 400."
        )
    ...
    def test_webhook_invalid_signature_returns_400(self, full_app_client):
        ...
        with patch("services.stripe_service.StripeService.verify_webhook_signature") as mock_verify:
            ...
            assert response.status_code == 400, (
                f"Webhook with invalid signature returned {response.status_code}, " f"expected 400."
            )
...
class TestAPIKeyProtection:
    ...
    def test_price_refresh_no_api_key_returns_401(self, full_app_client):
        ...
        assert response.status_code == 401, (
            f"Price refresh without API key returned {response.status_code}, " f"expected 401."
        )
    ...
    def test_price_refresh_wrong_api_key_returns_401(self, full_app_client):
        ...
        assert response.status_code == 401, (
            f"Price refresh with wrong API key returned {response.status_code}, " f"expected 401."
        )
    ...
    def test_price_refresh_jwt_as_api_key_returns_401(self, full_app_client):
        ...
        assert response.status_code == 401, (
            f"JWT accepted as API key on price refresh endpoint. " f"Got {response.status_code}."
        )
...
class TestOversizedRequests:
    ...
    def test_large_json_body_does_not_crash_server(self, client):
        ...
        assert response.status_code != 500, f"1 MB request body caused a 500 error."
    ...
    def test_deeply_nested_json_does_not_crash_server(self, client):
        ...
        assert response.status_code != 500, f"Deeply nested JSON caused a 500 error."
...
class TestSecurityHeadersOnRealApp:
    ...
    def test_api_endpoint_has_no_cache_headers(self, full_app_client):
        ...
        assert "no-store" in cache, (
            f"API endpoint missing 'no-store' cache directive. " f"Cache-Control: {cache!r}"
        )
