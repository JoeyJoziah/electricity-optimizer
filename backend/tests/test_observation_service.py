...
class TestGetModelAccuracyByVersion:
    ...
    async def test_multiple_versions_ranked(self, service, db):
        ...
        rows = [
            _row(
                model_version="v2.1", count=50, mape=3.45, rmse=0.012345, coverage=91.0
            ),
            _row(
                model_version="v2.0", count=80, mape=5.67, rmse=0.023456, coverage=85.0
            ),
            _row(
                model_version="v1.9", count=30, mape=8.90, rmse=0.034567, coverage=78.0
            ),
        ]
        db.execute = AsyncMock(return_value=_make_result(fetchall_value=rows))
