...
class TestGetPriceComparison:
    ...
    async def test_comparison_cheaper_than_national(self):
        ...
        mock_result = MagicMock()
        mock_result.mappings.return_value.all.return_value = [
            {"state": "CT", "price_per_gallon": 3.45, "fetched_at": datetime.now(UTC)},
            {"state": "US", "price_per_gallon": 3.50, "fetched_at": datetime.now(UTC)},
        ]
        mock_db.execute.return_value = mock_result
...
class TestIsHeatingOilState:
    ...
    def test_all_nine_states(self):
        ...
        assert expected == HEATING_OIL_STATES
