...
class FlatpeakClient(BasePricingClient):
    ...
    def _parse_price_response(
        ...
        return PriceData(
            ...
            source_api="flatpeak",
            energy_cost=(
                Decimal(str(breakdown.get("energy", 0)))
                if breakdown.get("energy")
                else None
            ),
            network_cost=(
                Decimal(str(breakdown.get("network", 0)))
                if breakdown.get("network")
                else None
            ),
            taxes=(
                Decimal(str(breakdown.get("taxes", 0)))
                if breakdown.get("taxes")
                else None
            ),
            levies=(
                Decimal(str(breakdown.get("levies", 0)))
                if breakdown.get("levies")
                else None
            ),
