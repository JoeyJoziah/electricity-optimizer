...
class PriceService:
    ...
    async def get_cheapest_supplier(self, region: PriceRegion) -> Price | None:
        """
        Find the cheapest supplier in a region.

        Uses a SQL ORDER BY ... LIMIT 1 query instead of fetching 50 rows
        and doing Python-side min() (19-P2-1).
        ...
        """
        return await self._repo.get_cheapest_price(region)
    ...
    async def get_price_forecast(
        self, region: PriceRegion, hours: int = 24, supplier: str | None = None
    ) -> PriceForecast | None:
        """
    ...
    async def _build_features(self, region: PriceRegion, _hours: int):
