...
- **DSP (Data Structure Protocol)**: `.dsp/` codebase graph — 470 entities (432 source + 38 external), 920 imports, 0 artificial cycles. CLI: `python3 dsp-cli.py --root . <command>`. UID map: `.dsp/uid_map.json`. Auto-discovery bootstrap: `python3 scripts/dsp_auto_bootstrap.py` (rebuilds from scratch in ~1s). Use `search`, `get-recipients`, `get-children --depth N` before refactoring
