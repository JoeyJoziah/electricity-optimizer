...
export default function GasRatesContent() {
  ...
  const { data: historyData, isLoading: historyLoading } = useGasHistory(
    region,
    90,
  );
  const { data: compareData, isLoading: compareLoading } =
    useGasSupplierComparison(ratesData?.is_deregulated ? region : null);

  const latestPrice = ratesData?.prices?.[0];
  const priceNum = latestPrice ? parseFloat(latestPrice.price) : null;

  // Derive trend from stats
  const avgPrice = statsData?.avg_price
    ? parseFloat(statsData.avg_price)
    : null;
              ...
              {statsLoading ? (
                ...
                <>
                  <p className="mt-2 text-3xl font-bold text-success-600">
                    {statsData?.min_price
                      ? formatCurrency(parseFloat(statsData.min_price))
                      : "--"}
                  </p>
                  <p className="mt-1 text-sm text-gray-500">
                    /{statsData?.unit || "therm"}
              ...
              {statsLoading ? (
                ...
                <>
                  <p className="mt-2 text-3xl font-bold text-danger-600">
                    {statsData?.max_price
                      ? formatCurrency(parseFloat(statsData.max_price))
                      : "--"}
                  </p>
                  <p className="mt-1 text-sm text-gray-500">
                    /{statsData?.unit || "therm"}
          ...
          <CardContent>
            {historyLoading ? (
              ...
              <div className="space-y-2">
                {historyData.prices.slice(0, 20).map((p) => (
                  <div
                    key={`${p.supplier}-${p.timestamp}-${p.price}`}
              ...
              <div className="flex h-48 items-center justify-center text-gray-500">
                No price history available yet. Rates are fetched weekly from
                EIA.
              </div>
            )}
          </CardContent>
