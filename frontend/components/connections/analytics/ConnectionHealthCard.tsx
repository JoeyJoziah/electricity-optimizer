...
export function ConnectionHealthCard({
  ...
  const totalIssues =
    (data?.stale_connections.length || 0) +
    (data?.rate_change_alerts.length || 0);
    ...
    <Card data-testid="connection-health-card">
      ...
      {state === "loading" && (
        <div
          ...
          data-testid="connection-health-loading"
          role="status"
        >
      ...
      {state === "success" && data && (
        <div className="space-y-3">
              ...
              >
                ...
                {onSync && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleSyncConnection(conn.connection_id)}
                    loading={syncingIds.has(conn.connection_id)}
                    disabled={syncingIds.has(conn.connection_id)}
                    aria-label={`Sync ${name}`}
                  >
                    <RefreshCw
                      className={cn(
                        "h-3.5 w-3.5 mr-1",
                        syncingIds.has(conn.connection_id) && "animate-spin",
                      )}
                    />
                    Sync Now
                  </Button>
          ...
          {/* Rate change alerts */}
          {data.rate_change_alerts.map((alert, idx) => {
            const isIncrease =
              alert.direction === "increase" ||
              alert.current_rate > alert.previous_rate;
            const supplierName =
              alert.supplier || alert.connection_label || "Supplier";

            return (
              <div
                key={`${alert.connection_id}-${alert.detected_at}-${idx}`}
                className={cn(
                ...
                data-testid={`rate-alert-${alert.connection_id}`}
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  >
                    {supplierName} rate {isIncrease ? "increased" : "decreased"}
                  </span>
                  <Badge
                ...
                >
                  {(alert.previous_rate * 100).toFixed(2)} c/kWh &rarr;{" "}
                  {(alert.current_rate * 100).toFixed(2)} c/kWh
                </p>
              </div>
            );
          })}
