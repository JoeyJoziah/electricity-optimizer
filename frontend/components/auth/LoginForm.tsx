 ...
 *
 * Provides email/password login with OAuth and magic link options.
...
export function LoginForm({ onSuccess }: LoginFormProps) {
  const { signIn, signInWithGoogle, signInWithGitHub, sendMagicLink, isLoading, error, clearError } = useAuth()
        ...
        <div className="bg-white rounded-xl shadow-card p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6 leading-relaxed">
              We&apos;ve sent a magic link to <strong className="text-gray-900">{email}</strong>. Click the link to sign in.
            </p>
            <button
              onClick={() => {
      ...
      <div className="bg-white rounded-xl shadow-card p-8">
        ...
        {(error || localError) && (
          ...
          >
            ...
            <div className="text-sm text-danger-700">
              ...
              {(error || localError)?.toLowerCase().includes('email not verified') && (
                <Link
                  href={`/auth/verify-email${email ? `?email=${encodeURIComponent(email)}` : ''}`}
                  className="mt-2 inline-block text-primary-600 hover:text-primary-700 font-medium underline"
                >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            ...
            {isLoading ? 'Signing in...' : showMagicLink ? 'Send magic link' : 'Sign in'}
          </button>
        </form>
        ...
        <div className="mt-6 text-center text-sm text-gray-600">
          ...
          <Link href="/auth/signup" className="text-primary-600 hover:text-primary-700 font-medium transition-colors duration-200">
            Sign up
          </Link>
        </div>
      </div>
