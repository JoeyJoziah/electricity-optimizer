...
export function SignupForm({ onSuccess }: SignupFormProps) {
  ...
  // Cloudflare Turnstile token (P1-7). Empty string blocks submit; the
  // widget emits a sentinel value when NEXT_PUBLIC_TURNSTILE_SITE_KEY is
  // unset so local development continues to work.
  ...
  const confirmPasswordMatch = Boolean(
    confirmPassword && password === confirmPassword,
  );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Turnstile gate — when configured, refuse submission until the user
    // has completed the challenge. NEXT_PUBLIC_TURNSTILE_SITE_KEY unset
    // emits the dev sentinel and falls through.
    if (!turnstileToken) {
      setLocalError(
    ...
    try {
      await signUp(email, password, name || undefined, { turnstileToken });
      onSuccess?.();
    } catch {
      ...
      <div className="bg-white rounded-xl shadow-card p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {[
                  ...
                  return (
                    <div key={key} className="flex items-center gap-2 text-xs">
                      ...
                      <span
                        className={met ? "text-success-700" : "text-gray-500"}
                      >
                        {label}
                      </span>
                    </div>
          ...
          <Input
            ...
            required
            error={
              confirmPasswordHasError ? "Passwords do not match" : undefined
            }
          ...
          {/* Cloudflare Turnstile (P1-7) — renders nothing when
