...
function mockNonJsonErrorResponse(
  status: number,
  statusText: string,
): Response {
  return {
...
describe("apiClient.get", () => {
  ...
  it("should throw ApiClientError with statusText when error body is not JSON", async () => {
    mockFetch.mockResolvedValue(
      mockNonJsonErrorResponse(500, "Internal Server Error"),
    );
...
describe("apiClient.put", () => {
  it("should make a PUT request with JSON body", async () => {
    ...
    mockFetch.mockResolvedValue(
      mockJsonResponse({ id: "1", name: "Updated Name" }),
    );
  ...
  it("should throw ApiClientError on error response", async () => {
    ...
    await expect(apiClient.put("/admin/settings", {})).rejects.toThrow(
      ApiClientError,
    );
...
describe("error handling edge cases", () => {
  it("should prefer detail field over message field in error response", async () => {
    mockFetch.mockResolvedValue(
      mockJsonResponse(
        { detail: "Detail error", message: "Message error" },
        400,
      ),
    );
  ...
  it("should use default message when error body has neither detail nor message", async () => {
    mockFetch.mockResolvedValue(mockJsonResponse({ code: "ERR_UNKNOWN" }, 400));
