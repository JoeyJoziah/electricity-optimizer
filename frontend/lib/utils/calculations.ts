...
export function calculatePriceTrend(
  ...
  const avgFirst =
    recentPrices
      .slice(0, Math.floor(recentPrices.length / 2))
      .reduce((a, b) => a + b, 0) / Math.floor(recentPrices.length / 2);

  const avgSecond =
    recentPrices
      .slice(Math.ceil(recentPrices.length / 2))
...
export function findOptimalPeriods(
  ...
  if (currentPeriod && pricesWithTime.length > 0) {
    const lastEntry = pricesWithTime[pricesWithTime.length - 1];
    const lastTime = lastEntry?.time ?? currentPeriod.start;
    optimalPeriods.push({
      start: currentPeriod.start,
      end: lastTime,
      avgPrice:
