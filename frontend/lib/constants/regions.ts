...
/** Flat list of all state options (includes international) */
...
/**
 * US state abbreviation -> full name map (50 states + DC, no international).
 * Single source of truth for all components that need abbr-to-name lookups.
 */
export const US_STATES: Record<string, string> = Object.fromEntries(
  US_REGIONS.filter((g) => g.label !== "International")
    .flatMap((g) => g.states)
    .map((s) => [s.abbr, s.label]),
);

/**
 * Sorted array of US state abbreviation codes (50 states + DC).
 * Use for simple <select> dropdowns that only need abbreviations.
 */
export const US_STATES_ABBR: string[] = Object.keys(US_STATES).sort();
