...
import VerifyEmailPage from '@/app/(app)/auth/verify-email/page'
...
describe('VerifyEmailPage — no token (check email screen)', () => {
  ...
  it('shows email input field when no email param', () => {
    render(<VerifyEmailPage />)
    const emailInput = screen.getByPlaceholderText(/enter your email to resend/i)
  ...
  it('calls sendVerificationEmail with email on resend', async () => {
    ...
    await waitFor(() => {
      expect(mockSendVerificationEmail).toHaveBeenCalledWith({ email: 'user@example.com' })
    })
  })
...
describe('VerifyEmailPage — with valid token', () => {
  ...
  it('calls authClient.verifyEmail with the token', async () => {
    ...
    await waitFor(() => {
      expect(mockVerifyEmail).toHaveBeenCalledWith({ query: { token: 'valid-token-123' } })
    })
  })
...
describe('VerifyEmailPage — with invalid token', () => {
  ...
  it('shows request new verification email link', async () => {
    ...
    await waitFor(() => {
      const link = screen.getByRole('link', { name: /request a new verification email/i })
