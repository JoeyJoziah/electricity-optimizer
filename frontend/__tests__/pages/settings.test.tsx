...
jest.mock("@/lib/store/settings", () => ({
  ...
  useSettingsStore: (selector?: (s: Record<string, unknown>) => unknown) =>
    selector
      ? selector(mockStoreState as Record<string, unknown>)
      : mockStoreState,
}));
...
jest.mock("@/components/suppliers/SupplierSelector", () => ({
  SupplierSelector: () => (
    <div data-testid="supplier-selector">SupplierSelector</div>
  ),
}));

jest.mock("@/components/suppliers/SupplierAccountForm", () => ({
  SupplierAccountForm: () => (
    <div data-testid="supplier-account-form">SupplierAccountForm</div>
  ),
}));
...
describe("SettingsPage", () => {
  ...
  it("calls setRegion when region changes", async () => {
    ...
    const regionSelect = document
      .querySelector('select optgroup[label="Northeast"]')
      ?.closest("select");
  ...
  it("renders current supplier info when supplier is set", () => {
    ...
    expect(
      screen.getAllByRole("button", { name: /change/i }).length,
  ...
  it("renders notification checkboxes", () => {
    ...
    expect(
      screen.getByLabelText(/optimal time reminders/i),
    ).toBeInTheDocument();
  ...
  it("calls setDisplayPreferences when currency changes", async () => {
    ...
    const currencySelect = selects.find(
      (s) => (s as HTMLSelectElement).value === "USD",
    )!;
  ...
  it("renders Privacy and Data section", () => {
    render(<SettingsPage />);
    expect(
      screen.getAllByText(/privacy & data/i).length,
    ).toBeGreaterThanOrEqual(1);
  });
  ...
  it("calls GDPR export endpoint and triggers download", async () => {
    ...
    await waitFor(() => {
      expect(mockFetch).toHaveBeenCalledWith("/api/v1/compliance/gdpr/export", {
        credentials: "include",
      });
    });
  ...
  it("calls delete endpoint when Delete My Account is confirmed via modal", async () => {
    mockFetch.mockResolvedValueOnce({ ok: true });

    const user = userEvent.setup();
    render(<SettingsPage />);

    // Click Delete My Account to open modal
    await user.click(
      screen.getByRole("button", { name: /delete my account/i }),
    );

    // Modal should appear with confirmation button
    await waitFor(() => {
    ...
    await waitFor(() => {
      expect(mockFetch).toHaveBeenCalledWith("/api/v1/compliance/gdpr/delete", {
        method: "DELETE",
        credentials: "include",
      });
    });
  ...
  it("does not call delete endpoint when Delete modal is cancelled", async () => {
    const user = userEvent.setup();
    render(<SettingsPage />);
