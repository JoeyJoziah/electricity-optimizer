...
describe("LandingPage", () => {
  ...
  it("renders the hero heading", () => {
    expect(
      screen.getByRole("heading", {
        level: 1,
        name: /save money on your electricity bills/i,
      }),
    ).toBeInTheDocument();
  });
  ...
  it("renders hero description text", () => {
    expect(
      screen.getByText(/AI-powered price optimization/i),
    ).toBeInTheDocument();
  });

  it('does NOT claim coverage across "all 50 states"', () => {
    expect(screen.queryByText(/all 50 states/i)).not.toBeInTheDocument();
  });

  it("describes service availability with deregulated market language", () => {
    // Phrase appears in both the hero paragraph and a feature card description
    const matches = screen.getAllByText(/deregulated electricity markets/i);
    expect(matches.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it("renders features section heading", () => {
    expect(
      screen.getByRole("heading", {
        level: 2,
        name: /everything you need to optimize energy costs/i,
      }),
    ).toBeInTheDocument();
  });
  ...
  it("renders all three pricing tiers", () => {
    expect(
      screen.getByRole("heading", { name: /^free$/i, level: 3 }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /^pro$/i, level: 3 }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /^business$/i, level: 3 }),
    ).toBeInTheDocument();
  });
  ...
  it("renders CTA links for all tiers", () => {
    ...
    expect(getStartedLinks.length).toBeGreaterThanOrEqual(1);
    expect(
      getStartedLinks.some((l) => l.getAttribute("href") === "/auth/signup"),
    ).toBe(true);
  ...
  it("renders footer with copyright text", () => {
    expect(
      screen.getByText(/rateshift. all rights reserved/i),
    ).toBeInTheDocument();
  });
