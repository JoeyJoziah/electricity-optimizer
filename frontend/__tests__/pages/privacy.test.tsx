...
describe("PrivacyPage", () => {
  ...
  it("renders all major section headings", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: /2\. how we use/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { level: 2, name: /3\. data storage/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { level: 2, name: /4\. gdpr compliance/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", {
        level: 2,
        name: /5\. third-party services/i,
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", {
        level: 2,
        name: /6\. automated decision-making/i,
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { level: 2, name: /7\. cookies/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { level: 2, name: /8\. data retention/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { level: 2, name: /9\. contact/i }),
    ).toBeInTheDocument();
  });
