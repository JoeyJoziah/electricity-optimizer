...
describe("useSettingsStore", () => {
  ...
  describe("setCurrentSupplier", () => {
    ...
    test("replaces existing supplier", () => {
      ...
      expect(useSettingsStore.getState().currentSupplier?.name).toBe(
        "NextEra Energy",
      );
    });
  });
  ...
  describe("appliance management", () => {
    ...
    test("addAppliance appends to list", () => {
      ...
      expect(useSettingsStore.getState().appliances).toHaveLength(2);
      expect(useSettingsStore.getState().appliances[0]!.name).toBe(
        "Dishwasher",
      );
      expect(useSettingsStore.getState().appliances[1]!.name).toBe(
        "Washing Machine",
      );
    });
    ...
    test("updateAppliance modifies specific appliance", () => {
      ...
      useSettingsStore
        .getState()
        .updateAppliance("app-1", { name: "Updated Dishwasher" });
    ...
    test("updateAppliance does not affect non-matching appliance", () => {
      ...
      useSettingsStore
        .getState()
        .updateAppliance("non-existent", { name: "Ghost" });
      expect(useSettingsStore.getState().appliances[0]!.name).toBe(
        "Dishwasher",
      );
    });
  ...
  describe("setNotificationPreferences", () => {
    test("updates partial notification preferences", () => {
      useSettingsStore
        .getState()
        .setNotificationPreferences({ supplierUpdates: true });
  ...
  describe("setDisplayPreferences", () => {
    test("updates currency", () => {
      ...
      expect(useSettingsStore.getState().displayPreferences.currency).toBe(
        "GBP",
      );
    });
    ...
    test("updates time format", () => {
      ...
      expect(useSettingsStore.getState().displayPreferences.timeFormat).toBe(
        "24h",
      );
    });
  ...
  describe("resetSettings", () => {
    test("resets all settings to defaults", () => {
      ...
      useSettingsStore
        .getState()
        .setDisplayPreferences({ currency: "GBP", theme: "dark" });
      useSettingsStore
        .getState()
        .setNotificationPreferences({ priceAlerts: false });
