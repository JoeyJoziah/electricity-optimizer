...
const defaultCurrentPricesResponse = {
  price: null,
  prices: [mockPriceResponse({ current_price: "0.25" })],
  region: "US_CT",
  timestamp: "2026-02-06T12:00:00Z",
  source: null,
};
...
jest.mock("@/lib/api/prices", () => ({
  getCurrentPrices: jest.fn(() =>
    Promise.resolve(defaultCurrentPricesResponse),
  ),
...
describe("Dashboard Integration", () => {
  ...
  it("renders dashboard with all main widgets", async () => {
    ...
    await waitFor(() => {
      ...
      // Price chart
      expect(
        screen.getByRole("img", { name: /price chart/i }),
      ).toBeInTheDocument();
  ...
  it("shows 24-hour forecast section", async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/24-hour forecast/i).length).toBeGreaterThan(
        0,
      );
    });
  });
  ...
  it("renders mobile-responsive layout structure", async () => {
    // Verify the dashboard renders its expected structural elements rather than
    // checking brittle CSS class names that can change with Tailwind refactors.
    render(<DashboardPage />, { wrapper });
    ...
    // Verify the stats row renders with expected content (semantic check)
    expect(screen.getAllByText(/current price/i).length).toBeGreaterThan(0);

    // Verify the dashboard has multiple distinct sections rendered
    const container = screen.getByTestId("dashboard-container");
    // The container should have child sections (stats, charts, forecast, etc.)
