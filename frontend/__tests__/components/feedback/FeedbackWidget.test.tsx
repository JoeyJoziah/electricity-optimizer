...
describe("FeedbackWidget", () => {
  ...
  // --- Error fallback message (non-Error throw) ---

  it("shows fallback message when fetch throws a non-Error value", async () => {
    // submitFeedback → fetch rejects with a plain string (not an Error)
    ...
    await user.type(
      screen.getByTestId("feedback-message"),
      "Long enough message to pass validation check here.",
    );
    await user.click(screen.getByTestId("feedback-submit"));
  ...
  // --- Error without detail field (fallback status message) ---

  it("shows status fallback when error response has no detail field", async () => {
    // json() returns empty object — body?.detail is undefined, so fallback message used
    mockFetch.mockResolvedValue({
      ok: false,
      status: 503,
      json: async () => ({}),
    ...
    await user.type(
      screen.getByTestId("feedback-message"),
      "Long enough message to pass minimum length check.",
    );
    await user.click(screen.getByTestId("feedback-submit"));
  ...
  it("Tab key at last focusable element wraps to first", async () => {
    const user = userEvent.setup();
    render(<FeedbackWidget />);
    ...
    // Tab through all elements until we reach the last, then Tab again to wrap
    await user.keyboard("{Tab}{Tab}{Tab}{Tab}{Tab}{Tab}{Tab}");
    // Just verify the modal is still open (no error thrown, focus stayed in modal)
    expect(screen.getByTestId("feedback-modal")).toBeInTheDocument();
  });

  it("Shift+Tab key at first focusable element wraps to last", async () => {
    const user = userEvent.setup();
    render(<FeedbackWidget />);
    ...
    // Shift+Tab from the first focused element should wrap to last
