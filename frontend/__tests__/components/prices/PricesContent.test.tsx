...
describe("PricesContent", () => {
  ...
  it('shows "Active" buttons when alerts are enabled', async () => {
    mockPriceAlerts = [
      { id: "1", type: "below", threshold: 0.2, enabled: true },
      { id: "2", type: "above", threshold: 0.3, enabled: true },
    ];
  ...
  it('calls removePriceAlert when "Active" button is clicked', async () => {
    mockPriceAlerts = [
      { id: "existing-alert", type: "below", threshold: 0.2, enabled: true },
    ];
  ...
  it("shows settings link text", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Configure more alerts in Settings/),
      ).toBeInTheDocument();
    });
  });
