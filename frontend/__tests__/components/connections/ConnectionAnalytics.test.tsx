...
describe("ConnectionAnalytics", () => {
  ...
  it("shows sync error banner when sync fetch returns !ok", async () => {
    mockAllAnalyticsFetches({
      health: {
        stale_connections: [
          {
    ...
    // The sync fetch returns a non-OK response
    ...
    const syncBtn = screen.getByRole("button", { name: /sync test supplier/i });
  ...
  it("shows sync error banner when sync fetch throws", async () => {
    mockAllAnalyticsFetches({
      health: {
        stale_connections: [
          {
    ...
    // The sync fetch throws a non-Error string
    mockFetch.mockRejectedValueOnce("network failure string");

    render(<ConnectionAnalytics />);

    await waitFor(() => {
      expect(screen.getByTestId("stale-connection-conn-2")).toBeInTheDocument();
    });

    fireEvent.click(
      screen.getByRole("button", { name: /sync test supplier 2/i }),
    );
  ...
  it("clears sync error on a second successful refresh", async () => {
    mockAllAnalyticsFetches({
      health: {
        stale_connections: [
          {
    ...
    // First sync fails to trigger the timeout path
    mockFetch.mockResolvedValueOnce({ ok: false, status: 503 });
    // Second sync (from refresh key) succeeds
    mockFetch.mockResolvedValue({ ok: true });

    render(<ConnectionAnalytics />);

    await waitFor(() => {
      expect(screen.getByTestId("stale-connection-conn-3")).toBeInTheDocument();
    });

    fireEvent.click(screen.getByRole("button", { name: /sync s3/i }));

    await waitFor(() => {
      expect(screen.getByRole("alert")).toBeInTheDocument();
    });

    // Clicking sync again clears timeout and resets error
    fireEvent.click(screen.getByRole("button", { name: /sync s3/i }));
    // A second failing sync would show the error again
  });
});
