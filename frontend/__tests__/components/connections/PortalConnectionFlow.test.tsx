...
describe('PortalConnectionFlow', () => {
  ...
  it('shows validation errors when form is submitted empty', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Please select a utility provider')).toBeInTheDocument()
    })

    expect(screen.getByText('Username is required')).toBeInTheDocument()
    expect(screen.getByText('Password is required')).toBeInTheDocument()
    expect(screen.getByText('You must consent before connecting')).toBeInTheDocument()
  })
  ...
  it('shows success state after successful connection and scrape', async () => {
    ...
    mockFetch.mockImplementation((url: string, options?: RequestInit) => {
      ...
      if (typeof url === 'string' && url.includes('/connections/portal') && options?.method === 'POST' && !url.includes('/scrape')) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve(mockPortalConnectionResponse),
        })
      }
      if (typeof url === 'string' && url.includes('/scrape') && options?.method === 'POST') {
        return Promise.resolve({
          ok: true,
    ...
    await user.type(screen.getByLabelText(/portal password/i), 'testpass')
    await user.click(screen.getByRole('checkbox'))
    await user.click(screen.getByRole('button', { name: /connect utility portal/i }))
  ...
  it('shows success state even when scrape fails', async () => {
    ...
    mockFetch.mockImplementation((url: string, options?: RequestInit) => {
      ...
      if (typeof url === 'string' && url.includes('/scrape') && options?.method === 'POST') {
        return Promise.resolve({
          ok: false,
          json: () => Promise.resolve({ detail: 'Scrape timeout' }),
        })
      }
      return Promise.resolve({ ok: true, json: () => Promise.resolve({}) })
    })
    ...
    expect(screen.getByText(/scrape will be retried automatically/i)).toBeInTheDocument()
  })
  ...
  it('shows error state when connection creation fails', async () => {
    ...
    mockFetch.mockImplementation((url: string, options?: RequestInit) => {
      ...
      if (typeof url === 'string' && url.includes('/connections/portal') && options?.method === 'POST') {
        return Promise.resolve({
          ok: false,
    ...
    mockFetch.mockImplementation((url: string, options?: RequestInit) => {
      ...
      if (typeof url === 'string' && url.includes('/connections/portal') && options?.method === 'POST') {
        ...
        if (callCount === 1) {
          return Promise.resolve({
            ok: false,
            json: () => Promise.resolve({ detail: 'Temporary error' }),
          })
        }
        return Promise.resolve({
          ok: true,
