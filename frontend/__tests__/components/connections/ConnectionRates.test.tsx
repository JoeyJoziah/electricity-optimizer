...
jest.mock("lucide-react", () => ({
  TrendingUp: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-trending" {...props} />
  ),
...
const { ApiClientError: MockApiClientError } =
  jest.requireMock("@/lib/api/client");
...
describe("ConnectionRates", () => {
  ...
  it("renders heading with supplier name", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Eversource Energy Rates")).toBeInTheDocument();
    });
  });
  ...
  it("shows empty state when no rates", async () => {
    ...
    expect(
      screen.getByText(/rate data will appear here after a successful sync/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows error state on fetch failure", async () => {
    mockApiGet.mockRejectedValueOnce(
      new Error("Failed to load rates. Please check your connection."),
    );
  ...
  it("shows upgrade gate on 403 error", async () => {
    ...
    expect(
      screen.getByText(/rate history is available on pro and business plans/i),
    ).toBeInTheDocument();
  });
