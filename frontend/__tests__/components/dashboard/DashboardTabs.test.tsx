...
jest.mock("next/dynamic", () => {
  return (loader: () => Promise<unknown>) => {
    ...
    if (src.includes("PropaneDashboard")) {
      const Comp = () => (
        <div data-testid="propane-dashboard">Propane Dashboard</div>
      );
    ...
    if (src.includes("WaterDashboard")) {
      const Comp = () => (
        <div data-testid="water-dashboard">Water Dashboard</div>
      );
    ...
    if (src.includes("CommunitySolarContent")) {
      const Comp = () => (
        <div data-testid="community-solar-dashboard">
...
describe("CombinedSavingsCard", () => {
  ...
  it("renders percentile badge", () => {
    renderWithProviders(<CombinedSavingsCard />);
    expect(screen.getByTestId("savings-percentile")).toHaveTextContent(
      "Top 85% of savers",
    );
  });
  ...
  it("shows loading skeleton when loading", () => {
    jest
      .spyOn(require("@/lib/hooks/useCombinedSavings"), "useCombinedSavings")
      .mockReturnValueOnce({
        data: null,
        isLoading: true,
        error: null,
...
describe("NeighborhoodCard", () => {
  ...
  it("shows insufficient data placeholder when percentile is null", () => {
    jest
      .spyOn(
        require("@/lib/hooks/useNeighborhood"),
        "useNeighborhoodComparison",
      )
      .mockReturnValueOnce({
...
describe("UtilityTabShell", () => {
  ...
  it("renders natural gas dashboard (GasRatesContent)", () => {
