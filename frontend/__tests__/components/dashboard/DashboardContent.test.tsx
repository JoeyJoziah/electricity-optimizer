...
jest.mock('next/dynamic', () => {
  ...
  const PriceLineChartMock = require('@/components/charts/PriceLineChart').PriceLineChart
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const ForecastChartMock = require('@/components/charts/ForecastChart').ForecastChart
...
jest.mock('@/components/charts/PriceLineChart', () => ({
  PriceLineChart: ({ data, timeRange, onTimeRangeChange, loading }: {
    data: unknown[]; timeRange: string; onTimeRangeChange: (r: string) => void; loading?: boolean
  }) => (
...
jest.mock('@/components/layout/Header', () => ({
  Header: ({ title }: { title: string }) => (
    <header data-testid="page-header"><h1>{title}</h1></header>
  ),
}))
...
jest.mock('@/components/ui/button', () => ({
  Button: ({ children, onClick, disabled, variant: _variant, size: _size, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: string; size?: string }) => (
    <button onClick={onClick} disabled={disabled} {...props}>{children}</button>
  ),
}))

jest.mock('@/components/suppliers/SupplierCard', () => ({
  SupplierCard: ({ supplier, isCurrent }: { supplier: { id: string; name: string }; isCurrent?: boolean }) => (
...
jest.mock('@/components/gamification/SavingsTracker', () => ({
  SavingsTracker: ({ streakDays, monthlySavings }: { streakDays: number; monthlySavings: number }) => (
...
jest.mock('lucide-react', () => ({
  ...
  Minus: (props: React.SVGAttributes<SVGElement>) => <svg data-testid="icon-minus" {...props} />,
  ArrowRight: (props: React.SVGAttributes<SVGElement>) => <svg data-testid="icon-arrow-right" {...props} />,
...
let mockCurrentSupplier: { id: string; name: string; estimatedAnnualCost: number } | null = null
...
function setDefaultHookReturns() {
  mockUseCurrentPrices.mockReturnValue({ data: defaultPriceData, isLoading: false, error: null })
  mockUsePriceHistory.mockReturnValue({ data: defaultHistoryData, isLoading: false })
  mockUsePriceForecast.mockReturnValue({ data: defaultForecastData, isLoading: false })
...
describe('DashboardContent', () => {
  ...
  it('renders loading skeleton when prices and history are loading', () => {
    mockUseCurrentPrices.mockReturnValue({ data: undefined, isLoading: true, error: null })
  ...
  it('renders -- for current price when no price data', () => {
    mockUseCurrentPrices.mockReturnValue({ data: { prices: [] }, isLoading: false, error: null })
  ...
  it('shows streak badge when streak_days > 0', () => {
    ...
    const streakBadge = successBadges.find((el) => el.textContent?.includes('-day streak'))
    ...
    mockUseSuppliers.mockReturnValue({
      data: {
        suppliers: [
          ...
          { id: 'sup_3', name: 'Third Supplier', avgPricePerKwh: 0.30, tariff_types: ['fixed'] },
        ],
      },
    })
  ...
  it('marks the current supplier card', () => {
    mockCurrentSupplier = { id: 'sup_1', name: 'Eversource Energy', estimatedAnnualCost: 1200 }
