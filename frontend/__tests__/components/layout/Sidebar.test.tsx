...
jest.mock("@/components/layout/NotificationBell", () => ({
  NotificationBell: () => (
    <div data-testid="notification-bell-sidebar">notifications</div>
  ),
}));

// Mock useAutoSwitcherPendingCount hook
let mockPendingCount: number | undefined = 0;
jest.mock("@/lib/hooks/useAutoSwitcher", () => ({
  useAutoSwitcherPendingCount: () => ({ data: mockPendingCount }),
}));
...
jest.mock("lucide-react", () => ({
  ...
  TrendingUp: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-prices" {...props} />
  ),
  Flame: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-gas-rates" {...props} />
  ),
  Sun: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-solar" {...props} />
  ),
  Building2: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-suppliers" {...props} />
  ),
  Link2: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-connections" {...props} />
  ),
  Calendar: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-optimize" {...props} />
  ),
  Bell: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-alerts" {...props} />
  ),
  ToggleLeft: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-auto-switcher" {...props} />
  ),
  Bot: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-assistant" {...props} />
  ),
  ...
  Fuel: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-fuel" {...props} />
  ),
  Droplets: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-droplets" {...props} />
  ),
  Waves: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-waves" {...props} />
  ),
  BarChart3: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-analytics" {...props} />
  ),
  Users: (props: React.SVGAttributes<SVGElement>) => (
    <svg data-testid="icon-community" {...props} />
  ),
  ...
  HelpCircle: (props: React.SVGAttributes<SVGElement>) => (
...
describe("Sidebar", () => {
  ...
  it("renders all 16 navigation items", () => {
    ...
    expect(screen.getByText("Auto Switcher")).toBeInTheDocument();
  ...
  it("renders navigation links with correct hrefs", () => {
    ...
    expect(screen.getByText("Prices").closest("a")).toHaveAttribute(
      "href",
      "/prices",
    );
    expect(screen.getByText("Gas Rates").closest("a")).toHaveAttribute(
      "href",
      "/gas-rates",
    );
    expect(screen.getByText("Heating Oil").closest("a")).toHaveAttribute(
      "href",
      "/heating-oil",
    );
    expect(screen.getByText("Propane").closest("a")).toHaveAttribute(
      "href",
      "/propane",
    );
    expect(screen.getByText("Water").closest("a")).toHaveAttribute(
      "href",
      "/water",
    );
    expect(screen.getByText("Solar").closest("a")).toHaveAttribute(
      "href",
      "/community-solar",
    );
    expect(screen.getByText("Suppliers").closest("a")).toHaveAttribute(
      "href",
      "/suppliers",
    );
    ...
    expect(screen.getByText("Optimize").closest("a")).toHaveAttribute(
      "href",
      "/optimize",
    );
    ...
    expect(screen.getByText("Auto Switcher").closest("a")).toHaveAttribute(
      "href",
      "/auto-switcher",
    );
    expect(screen.getByText("Assistant").closest("a")).toHaveAttribute(
      "href",
      "/assistant",
    );
  ...
  it("renders Auto Switcher between Alerts and Assistant in correct order", () => {
    render(<Sidebar />);

    const nav = screen.getByRole("navigation");
    const links = nav.querySelectorAll("a");
    const names = Array.from(links).map((link) => link.textContent?.trim());

    const alertsIdx = names.indexOf("Alerts");
    const autoSwitcherIdx = names.indexOf("Auto Switcher");
    const assistantIdx = names.indexOf("Assistant");

    expect(alertsIdx).toBeGreaterThan(-1);
    expect(autoSwitcherIdx).toBe(alertsIdx + 1);
    expect(assistantIdx).toBe(autoSwitcherIdx + 1);
  });
  ...
  it("highlights the active Auto Switcher route", () => {
    mockPathname = "/auto-switcher";
    render(<Sidebar />);

    const autoSwitcherLink = screen.getByText("Auto Switcher").closest("a");
    expect(autoSwitcherLink?.className).toContain("bg-primary-50");
  });
  ...
  describe("Auto Switcher badge", () => {
    it("does not show badge when pending count is 0", () => {
      mockPendingCount = 0;
      render(<Sidebar />);

      expect(
        screen.queryByTestId("auto-switcher-badge"),
      ).not.toBeInTheDocument();
    });

    it("shows badge with count when there are pending recommendations", () => {
      mockPendingCount = 3;
      render(<Sidebar />);

      const badge = screen.getByTestId("auto-switcher-badge");
      expect(badge).toBeInTheDocument();
      expect(badge).toHaveTextContent("3");
    });

    it("shows 99+ when count exceeds 99", () => {
      mockPendingCount = 150;
      render(<Sidebar />);
    ...
    it("does not show badge when pending count is undefined", () => {
      mockPendingCount = undefined;
      render(<Sidebar />);
