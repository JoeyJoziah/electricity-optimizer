...
const mockDeregulatedData = {
  count: 2,
  states: [
    { state_code: 'CT', state_name: 'Connecticut', puc_name: 'PURA', puc_website: null, comparison_tool_url: null },
    { state_code: 'OH', state_name: 'Ohio', puc_name: 'PUCO', puc_website: null, comparison_tool_url: null },
  ],
}
...
describe('useGasRates hooks', () => {
  ...
  it('useGasHistory returns history data', async () => {
    const { result } = renderHook(() => useGasHistory('us_ct', 30), { wrapper })
  ...
  it('useGasSupplierComparison returns suppliers', async () => {
    const { result } = renderHook(() => useGasSupplierComparison('us_ct'), { wrapper })
  ...
  it('useGasSupplierComparison(null) is disabled', () => {
    const { result } = renderHook(() => useGasSupplierComparison(null), { wrapper })
    expect(result.current.fetchStatus).toBe('idle')
