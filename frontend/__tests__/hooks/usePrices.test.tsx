...
describe("usePrices hooks", () => {
  ...
  it("useCurrentPrices returns price data", async () => {
    ...
    expect(mockGetCurrentPrices).toHaveBeenCalledWith(
      { region: "us_ct" },
      expect.anything(),
    );
  });
  ...
  it("usePriceHistory returns historical data", async () => {
    const { result } = renderHook(() => usePriceHistory("us_ct", 24), {
      wrapper,
    });
  ...
  it("usePriceHistory respects hours parameter", async () => {
    const { result } = renderHook(() => usePriceHistory("us_ct", 48), {
      wrapper,
    });
  ...
  it("usePriceForecast returns forecast data", async () => {
    const { result } = renderHook(() => usePriceForecast("us_ct", 24), {
      wrapper,
    });
    ...
    expect(result.current.data?.forecast.prices[0]!.price_per_kwh).toBe(
      "0.2300",
    );
    expect(mockGetPriceForecast).toHaveBeenCalledWith(
      { region: "us_ct", hours: 24 },
      expect.anything(),
    );
  });
  ...
  it("useOptimalPeriods returns periods", async () => {
    const { result } = renderHook(() => useOptimalPeriods("us_ct", 24), {
      wrapper,
    });
    ...
    expect(mockGetOptimalPeriods).toHaveBeenCalledWith(
      "us_ct",
      24,
      expect.anything(),
    );
  ...
  it("usePriceForecast(null) is disabled — no API call", () => {
    const { result } = renderHook(() => usePriceForecast(null, 24), {
      wrapper,
    });
  ...
  it("useOptimalPeriods(null) is disabled — no API call", () => {
    const { result } = renderHook(() => useOptimalPeriods(null, 24), {
      wrapper,
    });
