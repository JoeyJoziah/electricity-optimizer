...
describe("useCommunitySolar hooks", () => {
  ...
  it("useCommunitySolarPrograms returns programs", async () => {
    const { result } = renderHook(() => useCommunitySolarPrograms("NY"), {
      wrapper,
    });
    ...
    expect(result.current.data?.programs[0]!.program_name).toBe(
      "NY Community Solar",
    );
    expect(mockGetPrograms).toHaveBeenCalledWith(
      { state: "NY", enrollment_status: undefined },
      expect.anything(),
    );
  });
  ...
  it("useCommunitySolarPrograms passes enrollment filter", async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockGetPrograms).toHaveBeenCalledWith(
      { state: "NY", enrollment_status: "open" },
      expect.anything(),
    );
  });

  it("useCommunitySolarSavings returns savings data", async () => {
    const { result } = renderHook(() => useCommunitySolarSavings("150", "10"), {
      wrapper,
    });
  ...
  it("useCommunitySolarSavings(null, null) is disabled", () => {
    const { result } = renderHook(() => useCommunitySolarSavings(null, null), {
      wrapper,
    });
    expect(result.current.fetchStatus).toBe("idle");
  });
  ...
  it("useCommunitySolarSavings is disabled for negative monthly bill", () => {
    const { result } = renderHook(() => useCommunitySolarSavings("-50", "10"), {
      wrapper,
    });
    expect(result.current.fetchStatus).toBe("idle");
  ...
  it("useCommunitySolarSavings is disabled for negative savings percent", () => {
    const { result } = renderHook(() => useCommunitySolarSavings("150", "-5"), {
      wrapper,
    });
    expect(result.current.fetchStatus).toBe("idle");
    expect(mockGetSavings).not.toHaveBeenCalled();
  ...
  it("useCommunitySolarSavings is disabled for non-numeric bill", () => {
    const { result } = renderHook(() => useCommunitySolarSavings("abc", "10"), {
      wrapper,
    });
    expect(result.current.fetchStatus).toBe("idle");
    expect(mockGetSavings).not.toHaveBeenCalled();
  ...
  it("useCommunitySolarSavings allows zero values", async () => {
    const { result } = renderHook(() => useCommunitySolarSavings("0", "0"), {
      wrapper,
    });
  ...
  it("useCommunitySolarSavings allows boundary values", async () => {
    const { result } = renderHook(
      () =>
        useCommunitySolarSavings(
          String(MAX_MONTHLY_BILL),
          String(MAX_SAVINGS_PERCENT),
        ),
      { wrapper },
    );
  ...
  it("useCommunitySolarProgram returns a single program", async () => {
    const { result } = renderHook(() => useCommunitySolarProgram("1"), {
      wrapper,
    });
