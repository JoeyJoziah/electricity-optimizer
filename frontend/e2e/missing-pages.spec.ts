...
test.describe("Analytics Page", { tag: ["@regression"] }, () => {
  test("loads analytics page with heading visible", async ({
  ...
  test("shows state selector for chart area", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/analytics", { waitUntil: "domcontentloaded" });
  ...
  test("displays forecast and optimization sections", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/analytics", { waitUntil: "domcontentloaded" });
...
test.describe("ISR Rate Pages", { tag: ["@smoke"] }, () => {
  test("loads Connecticut electricity rate page", async ({ page }) => {
    ...
    await page.goto("/rates/connecticut/electricity", {
  ...
  test("loads California electricity rate page", async ({ page }) => {
    ...
    await page.goto("/rates/california/electricity", {
      waitUntil: "domcontentloaded",
    });
  ...
  test("loads Texas electricity rate page", async ({ page }) => {
    ...
    await page.goto("/rates/texas/electricity", {
      waitUntil: "domcontentloaded",
    });
  ...
  test("shows state name and utility type in page content", async ({
    page,
  }) => {
    await mockBetterAuth(page);
    await createMockApi(page);
  ...
  test("invalid state returns 404 or shows not-found content", async ({
    page,
  }) => {
    await mockBetterAuth(page);
    await createMockApi(page);
...
test.describe("OAuth Callback Page", { tag: ["@regression"] }, () => {
  test("shows loading state when landing on callback page", async ({
    page,
  }) => {
    await mockBetterAuth(page);
    await createMockApi(page);
    ...
    // The callback page either shows a loading spinner or redirects to dashboard.
    // Wait briefly for the page to settle, then check the actual state.
    await page.waitForTimeout(500);

    const onDashboard = page.url().includes("/dashboard");
    if (onDashboard) {
      // Redirected successfully — verify dashboard has meaningful content
      await expect(page.getByRole("heading").first()).toBeVisible({
        timeout: 5000,
      });
    } else {
      // Still on callback page — the spinner should be visible
      await expect(
        page.locator('[role="status"][aria-label="Completing authentication"]'),
      ).toBeVisible({ timeout: 5000 });
    }
  });
...
test.describe("Connections Page", { tag: ["@regression"] }, () => {
  test("loads connections page with heading", async ({
    authenticatedPage: page,
  }) => {
...
test.describe("Alerts Page — With Data", { tag: ["@smoke"] }, () => {
  ...
  test("shows alert list when alerts exist", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/alerts", { waitUntil: "domcontentloaded" });
  ...
  test("shows active and paused status badges", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/alerts", { waitUntil: "domcontentloaded" });
...
test.describe("Assistant Page", { tag: ["@regression"] }, () => {
  test("loads AI assistant page with heading", async ({
    authenticatedPage: page,
  }) => {
  ...
  test("shows chat input area", async ({ authenticatedPage: page }) => {
    ...
    await expect(page.getByPlaceholder(/ask rateshift ai/i)).toBeVisible();
  });
  ...
  test("shows example prompts in empty state", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/assistant", { waitUntil: "domcontentloaded" });
    ...
    await expect(page.getByText(/cheapest electricity rates/i)).toBeVisible();
  });

  test("shows RateShift AI header in chat panel", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/assistant", { waitUntil: "domcontentloaded" });
    ...
    await expect(page.getByText(/energy savings assistant/i)).toBeVisible();
  });
});
