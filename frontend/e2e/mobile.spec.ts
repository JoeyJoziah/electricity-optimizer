...
test.describe('Mobile Navigation', () => {
  ...
  test('mobile menu opens and reveals navigation links @regression', async ({
    ...
    await expect(
      page.getByRole('link', { name: /dashboard|prices|suppliers|alerts/i }).first()
...
test.describe('Mobile Forms', () => {
  ...
  test('login form is usable on mobile viewport @regression', async ({ page }) => {
    await mockBetterAuth(page)
    await clearAuthState(page)
    ...
    const emailWidth = await page.locator('#email').evaluate(
    ...
    const submitWidth = await submitButton.first().evaluate(
      (el) => (el as HTMLElement).getBoundingClientRect().width
    )
  ...
  test('settings form is usable on mobile viewport @regression', async ({ page }) => {
    await setupMobilePage(page)
    await page.goto('/settings', {
      waitUntil: 'domcontentloaded',
      timeout: 15000,
    ...
    if (selectCount > 0) {
      const firstSelectWidth = await selects.first().evaluate(
        (el) => (el as HTMLElement).getBoundingClientRect().width
      )
...
test.describe('Mobile Dashboard Content', () => {
  ...
  test('dashboard main widgets are visible on mobile @regression', async ({ page }) => {
    await setupMobilePage(page)
    await page.goto('/dashboard', {
      waitUntil: 'domcontentloaded',
      timeout: 15000,
    ...
    await expect(
      page.getByText('Current Price').first()
    ).toBeVisible({ timeout: 10000 })

    await expect(
      page.getByText('Total Saved')
    ).toBeVisible({ timeout: 10000 })
  })
