...
test.describe('Authentication Flows', () => {
  ...
  test('user can login with email and password', { tag: ['@smoke'] }, async ({ page }) => {
    await mockBetterAuth(page)
  ...
  test('shows error for invalid credentials', { tag: ['@regression'] }, async ({ page }) => {
    await mockBetterAuth(page)
    await page.goto('/auth/login')
  ...
  test('initiates OAuth flow with Google', { tag: ['@regression'] }, async ({ page }) => {
    await mockBetterAuth(page)
    await page.goto('/auth/login')
  ...
  test('user can logout', { tag: ['@smoke'] }, async ({ page, isMobile, browserName }) => {
  ...
  test('redirects unauthenticated users to login', { tag: ['@smoke'] }, async ({ page }) => {
    await mockBetterAuth(page)
  ...
  test('shows forgot password link', { tag: ['@regression'] }, async ({ page }) => {
    await mockBetterAuth(page)
    await page.goto('/auth/login')
  ...
  test('can navigate to signup from login', { tag: ['@regression'] }, async ({ page }) => {
    await mockBetterAuth(page)
    await page.goto('/auth/login')
  ...
  test('preserves redirect URL after login', { tag: ['@regression'] }, async ({ page }) => {
    await mockBetterAuth(page)
    // Ensure no session cookie
    await clearAuthState(page)
  ...
  test('session persists on page refresh', { tag: ['@regression'] }, async ({ page }) => {
  ...
  test('handles token expiration gracefully', { tag: ['@regression'] }, async ({ page }) => {
...
test.describe('Redirect Loop Prevention', () => {
  ...
  test('stale cookie does not cause redirect loop on login page', { tag: ['@regression'] }, async ({ page }) => {
    await mockBetterAuth(page)
  ...
  test('preserves original callbackUrl through 401 cycle', { tag: ['@regression'] }, async ({ page }) => {
    await mockBetterAuth(page)
...
test.describe('Authentication Security', () => {
  ...
  test('rate limits login attempts', { tag: ['@regression'] }, async ({ page }) => {
  ...
  // Sign-out button is in sidebar (hidden on mobile). Webkit sign-out redirect
  // exceeds the 30s test timeout due to slow cookie/session clearing in the engine.
  test('clears sensitive data on logout', { tag: ['@regression'] }, async ({ page, isMobile, browserName }) => {
    test.skip(isMobile === true, 'Sign-out button is in sidebar, hidden on mobile')
    test.skip(browserName === 'webkit', 'Webkit sign-out redirect exceeds test timeout')
    await setAuthenticatedState(page)
    await mockBetterAuth(page)
