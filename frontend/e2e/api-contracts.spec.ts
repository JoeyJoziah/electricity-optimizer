...
test.describe("API Contracts — Dashboard", () => {
  test("dashboard calls prices/current on load @smoke", async ({
    ...
    // Register listener before navigation so no requests are missed
    const pricesCurrentPromise = authenticatedPage.waitForRequest(
      (req) => req.url().includes("/prices/current"),
      { timeout: 10000 },
    );
    ...
    // Wait for React Query to fire its initial prices/current request
    await pricesCurrentPromise;

    const hasPricesCurrent = calledUrls.some((u) =>
      u.includes("/prices/current"),
    );
  ...
  test("dashboard calls prices/forecast on load @regression", async ({
    ...
    const forecastPromise = authenticatedPage.waitForRequest(
      (req) => req.url().includes("/prices/forecast"),
      { timeout: 10000 },
    );
  ...
  test("dashboard calls suppliers on load @regression", async ({
    authenticatedPage,
  }) => {
    const calledUrls: string[] = [];
    authenticatedPage.on("request", (req) => {
  ...
  test("dashboard calls savings/summary on load @regression", async ({
    ...
    const savingsPromise = authenticatedPage.waitForRequest(
      (req) => req.url().includes("/savings/summary"),
      { timeout: 10000 },
    );
...
test.describe("API Contracts — Prices Page", () => {
  test("prices page calls prices/current and prices/history @smoke", async ({
    ...
    const currentPromise = authenticatedPage.waitForRequest(
      (req) => req.url().includes("/prices/current"),
      { timeout: 10000 },
    );
    const historyPromise = authenticatedPage.waitForRequest(
      (req) => req.url().includes("/prices/history"),
      { timeout: 10000 },
    );
    ...
    await Promise.all([currentPromise, historyPromise]);
...
test.describe("API Contracts — Auth Headers", () => {
  test("requests to protected API endpoints include the session cookie @regression", async ({
    ...
    const authRequestPromise = authenticatedPage.waitForRequest(
      (req) =>
...
test.describe("API Contracts — Error States", () => {
  ...
  test("gated forecast endpoint returns 403 and shows upgrade prompt @regression", async ({
    ...
    // Wait for the page heading to confirm the shell rendered before asserting
    await expect(
    ...
    if (hasUpgradeText === 0) {
      console.log(
        "[api-contracts] Note: no upgrade prompt rendered for 403 forecast response",
      );
    }
  });
});
